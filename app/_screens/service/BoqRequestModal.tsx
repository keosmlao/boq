 ...
 */
export default function BoqRequestPage() {
  ...
  const fetchDocNo = async () => {
    ...
    try {
      const res = await fetch(`/api/rwpj/nextno`, {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `/api/locations?warehouse=${encodeURIComponent(selectedWarehouse)}`,
          { headers: _getAuthHeaders() },
        ).then((r) => r.json());
        const list = Array.isArray(res?.data) ? res.data : [];
        setLocations(list);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setLoading(true);
      try {
        const resp = await fetch(`/api/boq/${encodeURIComponent(docNo)}`, {
          headers: _getAuthHeaders(),
        }).then((r) => r.json());
        if (ignore) return;
  ...
  const saveRequest = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/requestsparepart`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ..._getAuthHeaders() },
