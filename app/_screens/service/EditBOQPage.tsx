...
const EditBOQPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const res = await fetch(`/api/boq/${encodeURIComponent(doc_no)}`, { headers: _getAuthHeaders() }).then(r => r.json());
  ...
  const fetchProducts = async (searchTerm) => {
    ...
    try {
      ...
      const res = await fetch(`/api/inventory?search=${encodeURIComponent(searchTerm)}`, { headers: _getAuthHeaders() }).then(r => r.json());
      const result = res;
      if (result.success) {
        setProducts(result);
