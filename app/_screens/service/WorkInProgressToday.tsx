...
export default function WorkInProgressToday() {
  ...
  const loadWorkOrders = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/work-orders?${new URLSearchParams({ status: 'in_progress' })}`, { headers: _getAuthHeaders() }).then(r => r.json());
      setOrders(res?.data || []);
    } catch (err) {
