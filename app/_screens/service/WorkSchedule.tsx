...
export default function WorkSchedule() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/work-schedule/${encodeURIComponent(projectCode || "")}/${encodeURIComponent(contractNo || "")}`, { headers: _getAuthHeaders() }).then(r => r.json());
  ...
  useEffect(() => {
    fetch("/api/tasks", { headers: _getAuthHeaders() }).then(r => r.json()).then(res => setTaskOptions(res?.data || [])).catch(() => {});
  }, []);
  ...
  const saveSchedule = async () => {
    ...
    try {
      ...
      body: JSON.stringify({ project_code: projectCode, contract_no: contractNo, tasks: rows }),
    });
