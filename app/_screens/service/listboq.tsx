...
export default function BOQTable() {
  ...
  const loadList = async (itemSearch?: string) => {
    ...
    setLoading(true);
    try {
      const params: Record<string, string> = { include_items: "0" };
      const itemSearchTrim = (itemSearch ?? debouncedItemFilter ?? "").trim();
      if (itemSearchTrim) params.item = itemSearchTrim;
      const resp = await fetch(`/api/listboq?${new URLSearchParams(params)}`, {
  ...
  const requestApprove = async (docNo: string, newStatus: 1 | 2) => {
    ...
    try {
      await fetch(`/api/projects/approveboq/${encodeURIComponent(docNo)}`, {
        method: "PUT",
  ...
  const confirmDelete = async () => {
    ...
    setDeleteDocNo(null);
    try {
      await fetch(`/api/boq/${encodeURIComponent(docNo)}/delete`, {
        method: "DELETE",
        headers: _getAuthHeaders(),
      });
    } catch (err) {
