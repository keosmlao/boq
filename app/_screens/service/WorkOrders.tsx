...
export default function WorkOrders() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/work-orders?${new URLSearchParams(params)}`, { headers: _getAuthHeaders() }).then(r => r.json());
      setOrders(res?.data || []);
    } catch (err) {
  ...
  const loadProjects = async () => {
    try {
      const res = await fetch("/api/projectsboq?contracts=1", { headers: _getAuthHeaders() }).then(r => r.json());
      const data = res?.data || [];
  ...
  const loadTechs = async () => {
    try {
      const res = await fetch("/api/technicians", { headers: _getAuthHeaders() }).then(r => r.json());
      const data = res?.data || [];
      if (data.length) {
        ...
        try {
          const helpersRes = await fetch("/api/helpers", { headers: _getAuthHeaders() }).then(r => r.json());
          const helpersData = helpersRes.data?.data || [];
      ...
      const fallback = await fetch("/api/tasks", { headers: _getAuthHeaders() }).then(r => r.json());
      const owners = Array.from(new Set((fallback.data?.data || []).map((t) => t.owner).filter(Boolean)));
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      try {
        const res = await fetch(`/api/work-schedule/${encodeURIComponent(form.project_code)}/${encodeURIComponent(form.contract_no)}`, { headers: _getAuthHeaders() }).then(r => r.json());
        const data = res?.data || [];
        if (Array.isArray(data) && data.length) setTaskOptions(data);
        else { const master = await fetch("/api/tasks", { headers: _getAuthHeaders() }).then(r => r.json()); setTaskOptions(master?.data || master?.data?.data || []); }
      } catch (err) {
        console.warn("Load tasks failed", err);
        try { const master = await fetch("/api/tasks", { headers: _getAuthHeaders() }).then(r => r.json()); setTaskOptions(master?.data || master?.data?.data || []); } catch {}
      }
    };
    fetchTasks();
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (techKey) {
        ...
        if (roworder) {
          ...
          if (currentDefaults !== nextDefaults) {
            await fetch(`/api/technicians/${roworder}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json", ..._getAuthHeaders() },
      body: JSON.stringify({ helpers: selHelpers }),
    });
      ...
      const response = await (editingId ? fetch(`/api/work-orders/${editingId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json", ..._getAuthHeaders() },
      ...
      if (res?.success) {
        ...
        setMessage(res?.message || "ການບັນທຶກລົ້ມເຫລວ");
      }
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try { await fetch(`/api/work-orders/${id}`, { method: "DELETE", headers: _getAuthHeaders() }); fetchOrders(); } catch (err) { console.error(err); }
  };
