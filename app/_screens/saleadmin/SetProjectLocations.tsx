...
export default function SetProjectLocations() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        const res = await fetch(`/api/projects/${id}`, { headers: _getAuthHeaders() }).then(r => r.json());
        const p = res?.data || {};
        setProject(p);
        setOffice({
          lat: p.office_lat ?? "",
        ...
        Swal.fire("Error", "Cannot load project", "error")
          .then(() => router.push("/sale-admin/listproject"));
      } finally {
        setLoading(false);
      }
    };
