  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (id) {
        ...
        // Roll the linked project forward to "ສະເໜີລາຄາ" stage. Best-effort —
        // swallow errors so a status-sync failure doesn't block the success
        // dialog or the navigation back to the list.
        try {
          await fetch(`/api/projects/${formData.project_id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json", ..._getAuthHeaders() },
            body: JSON.stringify({ project_status: "ສະເໜີລາຄາ" }),
          });
        } catch {
          /* ignore */
        }
