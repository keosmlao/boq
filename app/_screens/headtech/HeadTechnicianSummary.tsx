...
export default function HeadTechnicianSummary() {
  ...
  const loadData = async () => {
    ...
    try {
      const [woRes, techRes, helperRes] = await Promise.all([
        fetch("/api/work-orders", { headers: _getAuthHeaders() }).then((r) =>
          r.json(),
        ),
        fetch("/api/technicians", { headers: _getAuthHeaders() }).then((r) =>
          r.json(),
        ),
        fetch("/api/helpers", { headers: _getAuthHeaders() }).then((r) =>
          r.json(),
        ),
      ]);
      setOrders(woRes?.data || []);
      const techsData = techRes?.data || [];
      const helpers = helperRes?.data || [];
