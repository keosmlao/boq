  ...
  return (
    <Page max="max-w-none">
      <header className="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="mb-1 text-[11px] font-bold uppercase tracking-[0.16em] text-slate-700">{today || "ພາບລວມມື້ນີ້"}</p>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
              <div>
                <h2 className="flex items-center gap-2 text-[13px] font-black text-slate-900">
                  <ClipboardList size={16} className="text-slate-700" /> ວຽກທີ່ຕ້ອງດຳເນີນຕໍ່
                </h2>
        ...
        <aside className="space-y-5">
          <section className="overflow-hidden rounded-2xl bg-slate-950 p-5 text-white shadow-lg shadow-slate-950/10">
            <div className="flex items-center justify-between">
              ...
              <Link href="/reports" className="inline-flex items-center gap-1 text-[10.5px] font-bold text-slate-300 hover:text-white">ລາຍງານ <ArrowUpRight size={12} /></Link>
            </div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2 border-t border-white/10 pt-4">
              ...
              <div><p className="text-[10px] font-semibold text-slate-400">ການເຕີບໂຕ</p><p className="mt-1 text-[12px] font-bold text-slate-300">{loading ? "—" : `${Number(dashboard?.performance?.growth ?? 0)}%`}</p></div>
            </div>
          </section>
          ...
          {teams && (
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-3 flex items-center justify-between">
                <h2 className="text-[12px] font-black text-slate-900">ຄວາມວ່າງຂອງທີມຊ່າງ</h2>
                <Link href="/tech-teams" className="inline-flex items-center gap-1 text-[10.5px] font-bold text-slate-700 hover:text-slate-800">ຈັດການ <ArrowUpRight size={12} /></Link>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-2 py-2 text-center">
                  ...
                  <div className="text-[10px] font-bold text-slate-500">ວ່າງ</div>
                </div>
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-2 py-2 text-center">
                  <div className="text-lg font-black text-slate-800">{teams.busy}</div>
                  <div className="text-[10px] font-bold text-slate-500">ມີວຽກ</div>
                </div>
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-2 py-2 text-center">
                  <div className="text-lg font-black text-slate-800">{teams.working}</div>
                  <div className="text-[10px] font-bold text-slate-500">ກຳລັງເຮັດ</div>
                </div>
              </div>
              {teams.busy === 0 ? (
                <p className="mt-3 border-t border-slate-100 pt-3 text-center text-[11px] font-semibold text-slate-500">ທຸກທີມວ່າງ 🎉</p>
              ) : (
                <div className="mt-3 space-y-1 border-t border-slate-100 pt-3">
                  ...
                  {(teams.busyTeams?.length ?? 0) > 5 && <p className="px-1.5 pt-0.5 text-[10px] text-slate-400">+{teams.busyTeams.length - 5} ທີມ</p>}
                </div>
              )}
            </section>
          )}
          ...
          {install && (
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-3 flex items-center justify-between">
                <h2 className="text-[12px] font-black text-slate-900">ຕິດຕາມການຕິດຕັ້ງ</h2>
                <Link href="/install-tracking" className="inline-flex items-center gap-1 text-[10.5px] font-bold text-slate-700 hover:text-slate-800">ເບິ່ງທັງໝົດ <ArrowUpRight size={12} /></Link>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-2 py-2 text-center">
                  <div className="text-lg font-black text-slate-800">{install.active}</div>
                  <div className="text-[10px] font-bold text-slate-500">ກຳລັງດຳເນີນ</div>
                </div>
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-2 py-2 text-center">
                  <div className="text-lg font-black text-slate-800">{install.paused}</div>
                  <div className="text-[10px] font-bold text-slate-500">ພັກໂຄງການ</div>
                </div>
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-2 py-2 text-center">
                  <div className="text-lg font-black text-slate-800">{install.pendingPause}</div>
                  <div className="text-[10px] font-bold text-slate-500">ຄຳຮ້ອງລໍ</div>
                </div>
              </div>
              <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2.5 text-[11px]">
                <span className="font-semibold text-slate-400">ຊົ່ວໂມງເຮັດງານລວມ</span>
                <span className="font-black text-slate-800">{install.hours} ຊມ</span>
              </div>
                ...
                <div className="mt-2 space-y-1 border-t border-slate-100 pt-2">
                  {install.pausedProjects.map((p: any) => (
                    <Link key={p.project_id} href={`/projects/${p.project_id}`} className="flex items-center gap-2 rounded-lg px-1.5 py-1 hover:bg-slate-50">
                      ...
                      <span className="text-[10px] font-semibold text-slate-500">ພັກ {p.current_pause_days} ມື້</span>
                    </Link>
                  ))}
                </div>
              )}
