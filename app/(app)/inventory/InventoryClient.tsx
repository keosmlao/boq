 ...
 * waterfall. The debounced search refetch still runs client-side: the effect
 * skips its very first run (the seeded empty-search render) and fires for every
 * subsequent `search` change.
 */
...
export default function InventoryClient({ initialRows }: { initialRows: any[] }) {
  ...
  useEffect(() => {
    ...
    // (initialRows). Subsequent search changes still trigger a debounced refetch.
    if (!didMount.current) {
