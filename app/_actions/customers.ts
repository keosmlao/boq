...
export async function createCustomer(body: {
  ...
  try {
    ...
    let code = (body?.code || "").trim() || (body?.phone || "").trim();
    if (!code) {
      const d = new Date();
      const p = (n: number) => String(n).padStart(2, "0");
      code = `PJ${d.getFullYear()}${p(d.getMonth() + 1)}${p(d.getDate())}${p(d.getHours())}${p(d.getMinutes())}${p(d.getSeconds())}`;
    }
    // Dup-check + both inserts share one transaction so a failure can't leave an
    // orphan ar_customer with no detail row, and the MAX(roworder) reads happen
    // inside the same atomic unit.
    await withTransaction(async (client) => {
      const dup = await client.query(`SELECT code FROM ar_customer WHERE code = $1 LIMIT 1`, [code]);
      if (dup.rows.length) throw new Error("ລະຫັດ/ເບີໂທນີ້ມີໃນລະບົບແລ້ວ");
