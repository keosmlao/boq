  ...
  try {
    ...
    const result = await withTransaction(async (client) => {
      ...
      const existing = await client.query(
        `SELECT doc_no FROM odg_projects_boq WHERE contract_id = $1 LIMIT 1`,
        [contractId],
      );

      if (existing.rows[0]?.doc_no) {
        return { duplicate: true, doc_no: existing.rows[0].doc_no };
      }
      ...
      return { duplicate: false, doc_no: docNo };
    });

    if (result.missingContract) return fail("ບໍ່ພົບສັນຍາສຳລັບອອກ BOQ");
    if (result.duplicate) return fail("BOQ ຂອງສັນຍານີ້ມີຢູ່ແລ້ວ", { doc_no: result.doc_no });
