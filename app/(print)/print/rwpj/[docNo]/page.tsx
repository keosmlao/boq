...
import { getSparepartRequest } from "@/_actions/requests";
...
// Coerce a pg Date object (or any value) into a printable string so React can render it.
...
function PrintRWPJ() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await getSparepartRequest(String(docNo));
        setData(res || null);
      } catch (e) {
        console.error(e);
      }
    ...
    <div style={{ maxWidth: '210mm', margin: '0 auto', padding: '5mm', background: '#fff', fontFamily: 'var(--font-lao), sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5mm', marginBottom: '5mm', fontSize: '12pt' }}>
        <div>
          ...
          <div style={{ marginBottom: '2mm' }}><b>ວັນທີ:</b> {fmtDate(data.doc_date)}</div>
