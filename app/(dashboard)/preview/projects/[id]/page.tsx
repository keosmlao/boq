...
/**
 * PREVIEW / PROTOTYPE — read-only project pipeline page.
 *
 * Loads one project + its contracts/BOQ (getProjectsBoq) and its quotations
 * (getQuotations), derives the 7-stage pipeline, and renders the new
 * project-centric UX. Action buttons link to the EXISTING real pages — this
 * page itself performs no writes.
 */
...
function PipelinePage() {
  ...
  const boqs = useMemo(
    () =>
      contracts.flatMap((c) =>
        (Array.isArray(c?.boq_list) ? c.boq_list : []).map((b: any) => ({
          ...b,
  ...
  const ACTION_LINK: Partial<Record<string, { label: string; href: string }>> = {
    quotation: { label: "ໄປໜ້າໃບສະເໜີລາຄາ", href: "/sale-admin/quotations" },
    contract: { label: "ໄປໜ້າສັນຍາ", href: "/service-admin/list-project" },
    boq: { label: "ໄປໜ້າ BOQ", href: "/boq" },
  };
  ...
  if (!project) {
    return (
      <div className="mx-auto max-w-[1100px] px-4 py-10 text-center text-[var(--theme-text-mute)]">
        ບໍ່ພົບໂຄງການ
        <div className="mt-3">
          <button onClick={() => router.push("/preview/projects")} className="text-[var(--theme-primary)] hover:underline">
            ← ກັບໄປລາຍການ
          </button>
        </div>
      </div>
    ...
    <div className="bg-[var(--theme-page)] px-3 py-3 md:px-4">
      <div className="mx-auto max-w-[1100px]">
        <div className="mb-3 flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[12px] text-amber-800">
          ...
          <span><b>ຕົວຢ່າງ (Preview)</b> — read-only, ດຶງຂໍ້ມູນຈິງ, ບໍ່ກະທົບລະບົບ.</span>
        </div>

        <button
          onClick={() => router.push("/preview/projects")}
        ...
        <div className="theme-card mb-3 p-4">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              <h1 className="text-[18px] font-bold text-[var(--theme-text)]">
        ...
        {current && (
          <div className="theme-card mb-3 border-l-4 border-[var(--theme-primary)] p-4">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-[var(--theme-text-mute)]">
              ຂັ້ນຕອນປັດຈຸບັນ
            </div>
            <div className="mt-1 flex flex-wrap items-center justify-between gap-2">
              <div>
                <div className="text-[16px] font-bold text-[var(--theme-primary)]">{current.label}</div>
                <div className="text-[12px] text-[var(--theme-text-soft)]">{current.detail}</div>
              </div>
              {ACTION_LINK[current.key] && (
                <button
                  onClick={() => router.push(ACTION_LINK[current.key]!.href)}
                  className="inline-flex items-center gap-1.5 rounded-md bg-[var(--theme-primary)] px-3 py-2 text-[12px] font-semibold text-white hover:bg-[var(--theme-primary-strong)]"
                >
                  {ACTION_LINK[current.key]!.label}
          ...
          <div className="flex gap-1 border-b border-[var(--theme-border-subtle)] px-2">
            {tabs.map((t) => (
              ...
              >
                {t.label}
                {typeof t.n === "number" && (
                  <span className="ml-1 text-[10px] text-[var(--theme-text-mute)]">({t.n})</span>
                )}
...
function MiniTable({ head, rows, empty }: { head: string[]; rows: (string | number)[][]; empty: string }) {
  if (!rows.length) {
    return <div className="py-8 text-center text-[12px] text-[var(--theme-text-mute)]">{empty}</div>;
  }
  return (
