...
import { getContract, updateContract, deleteContract as deleteContractAction } from "@/_actions/contracts";
...
function ContractDetail() {
  ...
  const saveContract = async () => {
    ...
    try {
      ...
      const payload: any = await updateContract(String(contract.id), {
        ...contract,
        ...form,
        subtotal: total,
        total_amount: total,
  ...
  const deleteContract = async () => {
    ...
    try {
      const res: any = await deleteContractAction(String(contract.id));
      if (!res?.success) throw new Error();
      Swal.fire({ icon: "success", title: "ລົບແລ້ວ", timer: 1000, showConfirmButton: false });
      router.push(contract.quotation_id ? `/sale-admin/quotation/${contract.quotation_id}` : "/sale-admin/list-project");
    } catch {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError("");
        const payload: any = await getContract(String(id));
        if (payload?.success === false) throw new Error(payload?.message || "Contract not found");
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-[12px]">
                <button
                  type="button"
                  onClick={() => router.push(`/sale-admin/list-project?projectId=${encodeURIComponent(String(contract.project_id || ""))}`)}
