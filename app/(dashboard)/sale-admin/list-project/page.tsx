...
import { getProjects, getProject, updateProjectAction, deleteProjectAction } from "@/_actions/projects";
...
function ProjectList() {
  ...
  const toggleExpand = async (project: ProjectItem) => {
    ...
    try {
      ...
      if (contractsStale) {
        tasks.push((async () => {
        const j: any = await getProject(String(id), { includeContracts: true });
        const data = j?.data;
      ...
      if (!projectQuotations[String(id)]) {
        tasks.push((async () => {
          const qj: any = await getQuotations({ projectId: String(id) });
