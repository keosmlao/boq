...
function ManageTechnicians() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setTechs(tRes?.success ? tRes.data : []);
      setHelpers(hRes?.success ? hRes.data.data : []);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = form.roworder
        ? await updateTechnician(form.roworder, payload)
  ...
  const deleteMember = async (record) => {
    ...
    setMessage("");
    try {
      const res = await deleteTechnician(record.roworder);
      if (res?.success) {
