...
export default function ProjectForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.success) {
        if (mode === "edit") {
          router.push(`/v2/projects/${projectId}`);
        } else {
          // Continue the pipeline: register -> survey -> quotation.
          router.push(res.id ? `/v2/projects/${res.id}/survey/new` : "/v2/projects");
        }
      } else {
  ...
  const backHref = mode === "edit" && projectId ? `/v2/projects/${projectId}` : "/v2/projects";
