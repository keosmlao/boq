...
export default function CreateContractPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.success) {
        if (editId) {
          router.push(`/v2/contracts/${editId}`);
          return;
        }
        await advanceProjectStage(String(id), "ສັນຍາ").catch(() => {});
        router.push(`/v2/projects/${id}?tab=contracts`);
      ...
      {!editId && hasContract ? (
        <Card className="p-6 text-center">
          ...
          <div className="mt-4 flex justify-center">
            <Btn onClick={() => router.push(`/v2/projects/${id}`)}>ກັບໄປເບິ່ງສັນຍາ</Btn>
          </div>
        </Card>
        ...
        <Card className="p-6 text-center">
          ...
          <div className="mt-4 flex justify-center gap-2">
            ...
            <Btn onClick={() => router.push(`/v2/projects/${id}/quotation/new`)}>ສ້າງໃບສະເໜີ</Btn>
          </div>
        </Card>
      ) : (
        <form onSubmit={submit}>
          ...
          <div className="flex justify-end gap-2">
            <Btn type="button" variant="outline" onClick={() => router.push(`/v2/projects/${id}`)}>ຍົກເລີກ</Btn>
