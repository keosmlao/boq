...
export default function V2PipelinePage() {
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      if (res?.success) router.push("/v2/projects");
      else {
        setDeleting(false);
        setConfirmDel(false);
      }
  ...
  const stageAction: Partial<Record<string, { label: string; href: string }>> = {
    survey: { label: "ບັນທຶກສຳຫຼວດ", href: `/v2/projects/${id}/survey/new` },
    quotation: { label: "ສ້າງໃບສະເໜີລາຄາ", href: `/v2/projects/${id}/quotation/new` },
    contract: { label: "ສ້າງສັນຍາ", href: `/v2/projects/${id}/contract/new` },
    boq: { label: "ສ້າງ BOQ", href: `/v2/projects/${id}/boq/new` },
    taskplan: { label: "ກຳນົດໜ້າວຽກ", href: `/v2/projects/${id}/tasks/new` },
    workorder: { label: "ອອກໃບງານ", href: `/v2/projects/${id}/workorder/new` },
  };
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-700 via-blue-600 to-blue-500 p-5 text-white shadow-[var(--theme-shadow-lg)]">
            ...
            <div className="relative">
              ...
              <div className="mt-4 flex gap-2 border-t border-white/10 pt-4">
                <button
                  onClick={() => router.push(`/v2/projects/${id}/edit`)}
        ...
        <div className="min-w-0 rounded-2xl border border-slate-200/70 bg-white overflow-hidden shadow-xs">
          ...
          {tab === "survey" && (
            <div>
              <div className="mb-4 flex justify-end">
                <button
                  onClick={() => router.push(`/v2/projects/${id}/survey/new`)}
                  className={addBtn}
                >
          ...
          {tab === "quotations" && (
            <div>
              {!hasActiveQuo && (
                <div className="mb-4 flex justify-end">
                  <button
                    onClick={() => router.push(`/v2/projects/${id}/quotation/new`)}
                    className={addBtn}
                  >
          ...
          {tab === "contracts" && (
            <div>
              {!hasAnyContract && (
                <div className="mb-4 flex justify-end">
                  <button
                    onClick={() => router.push(`/v2/projects/${id}/contract/new`)}
                    className={addBtn}
                  >
          ...
          {tab === "boq" && (
            <div>
              {hasAnyContract && (
                <div className="mb-4 flex justify-end">
                  <button
                    onClick={() => router.push(`/v2/projects/${id}/boq/new`)}
                    className={addBtn}
                  >
            ...
            <div>
              {hasAnyContract && (
                <div className="mb-4 flex justify-end gap-2">
                  ...
                  <button
                    onClick={() => router.push(`/v2/projects/${id}/tasks/new`)}
                    className={addBtn}
                  >
          ...
          {tab === "workorders" && (
            <div>
              <div className="mb-4 flex justify-end">
                <button
                  onClick={() => router.push(`/v2/projects/${id}/workorder/new`)}
                  className={addBtn}
                >
          ...
          {tab === "requests" && (
            <div>
              {materials.length > 0 && (
                <div className="mb-4 flex justify-end">
                  <button
                    onClick={() => router.push(`/v2/projects/${id}/request/new`)}
                    className={addBtn}
                  >
            ...
            return (
              <tr key={q.id ?? i} className="group transition-colors hover:bg-slate-50/50">
                <td className="px-4 py-3 font-mono font-bold">
                  <Link href={`/v2/quotations/${q.id}`} className="text-blue-600 hover:text-blue-700 hover:underline">{q.quotation_no || "-"}</Link>
                </td>
          ...
          {requests.map((r, i) => {
            ...
            return (
              <tr key={r.id ?? i} className="group transition-colors hover:bg-slate-50/50">
                <td className="px-4 py-3 font-mono font-bold">
                  <Link href={`/v2/requests/${encodeURIComponent(r.id)}`} className="text-blue-600 hover:text-blue-700 hover:underline">{r.request_no || "-"}</Link>
        ...
        <tbody className="divide-y divide-slate-100">
          {workorders.map((w, i) => (
            <tr key={w.id ?? i} className="group transition-colors hover:bg-slate-50/50">
              <td className="px-4 py-3 font-mono font-bold">
                <Link href={`/v2/work-orders/${w.id}`} className="text-blue-600 hover:text-blue-700 hover:underline">{w.work_no || "-"}</Link>
              </td>
          ...
          {boqs.map((b, i) => {
            ...
            return (
              <tr key={docNo || i} className="group transition-colors hover:bg-slate-50/50">
                <td className="px-4 py-3 font-mono font-bold">
                  <Link href={`/v2/boq/${encodeURIComponent(docNo)}`} className="text-blue-600 hover:text-blue-700 hover:underline">{docNo || "-"}</Link>
                </td>
          ...
          <div key={c.id ?? c.contract_no ?? i} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs hover:shadow-xs transition-shadow">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-3">
              ...
              <div className="flex items-center gap-2.5">
                <Link
                  href={isErp ? `/v2/contracts/${encodeURIComponent(c.contract_no || "")}` : `/v2/contracts/${c.id}`}
