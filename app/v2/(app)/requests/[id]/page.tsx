  ...
  return (
    <Page max="max-w-none">
      <div className="mb-3 flex items-center justify-between gap-2">
        <button
          onClick={() => router.push("/v2/requests")}
        ...
        {!withdrawn && (
          <DocActions
            editHref={r.project_id ? `/v2/projects/${r.project_id}/request/new?edit=${encodeURIComponent(String(id))}` : undefined}
            onDelete={() => deleteRequest(String(id))}
            afterDelete="/v2/requests"
            label="ໃບຂໍເບີກ"
          />
        )}
      </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-4 shadow-sm bg-slate-50/50">
            <div className="flex flex-col gap-2.5">
              ...
              {r.project_id && (
                <button
                  onClick={() => router.push(`/v2/projects/${r.project_id}`)}
