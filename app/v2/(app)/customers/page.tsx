...
import { Page, PageHeader, Card, Btn } from "../_components/ui";
...
const META: Record<string, Meta> = {
  quotation: { label: "ໃບສະເໜີລາຄາ", icon: <FileText size={13} />, primary: (q) => q.quotation_no || "(ບໍ່ມີເລກທີ່)", secondary: (q) => [d10(q.quotation_date), q.total_amount ? `${money(q.total_amount)} ກີບ` : ""].filter(Boolean).join(" · "), status: (q) => quoStatus(q.status), href: (q) => `/v2/quotations/${q.id}` },
  contract: { label: "ສັນຍາ", icon: <FileSignature size={13} />, primary: (c) => c.contract_no || "(ບໍ່ມີເລກທີ່)", secondary: (c) => [c.total_amount ? `${money(c.total_amount)} ກີບ` : "", d10(c.sign_date)].filter(Boolean).join(" · "), status: (c) => contractStatus(c), href: (c) => (c.src === "erp" ? `/v2/contracts/${encodeURIComponent(c.contract_no || "")}` : `/v2/contracts/${c.id}`) },
  boq: { label: "BOQ", icon: <ListChecks size={13} />, primary: (b) => b.doc_no || b.boq_no || "(ບໍ່ມີເລກທີ່)", secondary: (b) => d10(b.doc_date), status: (b) => boqStatus(b), href: (b) => `/v2/boq/${encodeURIComponent(b.doc_no || b.boq_no || "")}` },
  request: { label: "ໃບຂໍເບີກ", icon: <PackageOpen size={13} />, primary: (r) => r.request_no || "(ບໍ່ມີເລກທີ່)", secondary: (r) => [d10(r.created_at), Array.isArray(r.items) ? `${r.items.length} ລາຍການ` : ""].filter(Boolean).join(" · "), status: (r) => reqStatus(r), href: (r) => `/v2/requests/${encodeURIComponent(r.id)}` },
  tasks: { label: "ໜ້າວຽກ", icon: <CalendarRange size={13} />, primary: (t) => t.title || "ໜ້າວຽກ", secondary: (t) => [t.phase, t.technician_name].filter(Boolean).join(" · "), status: () => null, href: (_t, pid) => `/v2/projects/${pid}?tab=tasks` },
  workorder: { label: "ໃບງານ", icon: <Wrench size={13} />, primary: (w) => w.work_no || "(ບໍ່ມີເລກທີ່)", secondary: (w) => [d10(w.work_date || w.created_at), w.technician_name].filter(Boolean).join(" · "), status: () => null, href: (w) => `/v2/work-orders/${w.id}` },
};
    ...
    <Page max="max-w-none w-full">
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4 border-b border-slate-100 pb-5">
        ...
        <div className="flex flex-shrink-0 items-center gap-2">
          ...
          <button
            onClick={() => router.push("/v2/customers/new")}
                  ...
                  <div key={code} className="group transition-colors">
                    ...
                    >
                      ...
                      <div className={`${ACT} opacity-0 group-hover:opacity-100 transition-opacity duration-150`} onClick={(e) => e.stopPropagation()}>
                        <button
                          onClick={() => router.push(`/v2/customers/new?edit=${encodeURIComponent(code)}`)}
                          title="ແກ້ໄຂ"
                      ...
                      <div className="divide-y divide-slate-100/70 border-t border-slate-100 bg-slate-50/20">
                        {c.projects.length === 0 ? (
                          <div style={{ paddingLeft: 30 }} className="flex items-center justify-between gap-3 py-2.5 pr-4">
                            ...
                            <button
                              onClick={() => router.push(`/v2/projects/new?cust=${encodeURIComponent(code)}&name=${encodeURIComponent(c.name || code)}`)}
